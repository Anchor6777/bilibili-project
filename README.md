...
<u>设置指数退避策略、最大重连次数的目的：</u>

限制重连的频率和次数，节省客户端和服务端的资源占用。频繁的重连会占用cpu资源，网络资源，造成客户端卡顿、耗电快，增加服务端负担等问题。

在网络持续瘫痪的场景，重连是没意义的，所以在重连超过一定次数时，停止重试，切换到网络故障页面，提供手动重连功能，让用户决定重连的时机。
